    ...
    for mid_1 in model_specs:
        for mid_2 in model_specs:
            try:
                ...
                k3 = '-gap' if gap_positive else '+gap'
                k4 = '-measure' if measure_diff_positive else '+measure'
                k5 = make_key_from_models(model_specs[mid_2], model_specs[mid_1], conditioning_hparams)
                table[a0[k0], a1[k1], a2[k2]] += 1
                table[a0[k3], a1[k4], a2[k5]] += 1
                count += 1
            except KeyError as e:
                print(e)
                continue
...
def entropy(p):
    ...
    if np.isnan(-np.sum(normalized_p * log)):
        pass
        # print('p: ', p, 'logp: ', log, 'H: ', -np.sum(normalized_p * log)) //TODO what happens here
...
def total_mi_from_table(table):
    ...
    expected_ce /= table.shape[-1]
    return expected_mi / expected_ce


def conditional_mutual_information_v2(prediction, model_specs):
    model_specs_copy = {}
...
"""
Prediction is a dictionary with key equal to 'model_x' where x is the number of the model and value the predicted value
for the generalization gap
"""


def get_mutual_information(prediction, task_number, google_folder_abs):
    configs_filepath_model = '{}/public_data/reference_data/{}/model_configs.json'.format(google_folder_abs, '{}')
    if task_number == 1:
        model_specs_filepath = configs_filepath_model.format('task1_v4')
    elif task_number == 2:
        model_specs_filepath = configs_filepath_model.format('task2_v1')
    else:
        raise Exception("This task does not exist")
    with open(model_specs_filepath, 'r') as f:
        model_specs = json.load(f)
    model_numbers = set(map(lambda key: key[6:], prediction.keys()))
    model_specs_for_prediction = {key: value for key, value in model_specs.items() if key in model_numbers}
    assert len(model_specs_for_prediction) == len(prediction)
    return conditional_mutual_information_v2(prediction, model_specs_for_prediction)
